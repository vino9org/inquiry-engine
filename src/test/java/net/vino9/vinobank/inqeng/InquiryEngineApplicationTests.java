...
class InquiryEngineApplicationTests {
    ...
    void test_get_account_details() {
        var query = String.format(" { CasaAccount(accountId:\"%s\") { accountId transactions { refId } } }", "123");
        ...
        List<String> refIds = result.read("data.CasaAccount.transactions[*].refId");
        assertThat(refIds).contains("10000001");
    }

    @Test
    void test_get_accounts_by_customer_id() {
        var query = String.format(" { CasaAccountsByCustomer(customerId: \"%s\") { accountId currency transactions { refId } } }", "111");
        var result = dgsQueryExecutor.executeAndGetDocumentContext(query);

        List<String> currencies = result.read("data.CasaAccountsByCustomer[*].currency");
        assertThat(currencies).contains("SGD").contains("USD");

        List<String> refIds = result.read("data.CasaAccountsByCustomer[*].transactions[*].refId");
        assertThat(refIds).contains("10000001").contains("10000002");
    }

    @Test
    void test_non_existent_account() {
        var query = String.format(" { CasaAccount(accountId:\"%s\") { accountId transactions { refId } } }", "bad_bad");
    ...
    void test_non_existent_customer() {
        var query = String.format(" { CasaAccountsByCustomer(customerId: \"%s\") { accountId currency transactions { refId } } }", "007");
